...
life_t *life_create(int size_y, int size_x)
{
	...
	life->cell_matrix_cur = (unsigned char **)malloc(sizeof(unsigned char *) * size_y);
	life->cell_matrix_new = (unsigned char **)malloc(sizeof(unsigned char *) * size_y);
	for (i = 0; i < size_y; i++) {
		life->cell_matrix_cur[i] = (unsigned char *)malloc(sizeof(unsigned char) * size_x);
		life->cell_matrix_new[i] = (unsigned char *)malloc(sizeof(unsigned char) * size_x);
	}
...
static void life_evolve_cell(life_t *life, int y, int x)
{
	...
	life->cell_matrix_new[y][x] = (n == 3 || (n == 2 && life->cell_matrix_cur[y][x]));
}
